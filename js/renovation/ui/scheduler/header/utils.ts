    ...
    switch (defaultElementType) {
      case VIEW_SWITCHER:
        if (useDropDownViewSwitcher) {
          return getDropDownViewSwitcher(item as ToolbarItem, selectedView, views, setCurrentView, rtlEnabled);
        }

        return getViewSwitcher(item as ToolbarItem, selectedView, views, setCurrentView, rtlEnabled);
      case DATE_NAVIGATOR:
